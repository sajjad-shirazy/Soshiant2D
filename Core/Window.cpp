...
namespace Soshiant2D {
	...
	Window::Window( std::string name , int width , int height , int colorDepth , bool isFullscrean )
	{
		this->title = name;
		this->width = width;
		this->height = height;
	...
	HWND Window::Init(HINSTANCE hInstance,int nCmdShow)
	{
	...
	void Window::setConsol(bool value)
	{
		if(value)
			AllocConsole();
		else
