...
namespace Soshiant2D {
	...
	public:
		Sprite(Layer* parent);
		Sprite(std::string path,Layer* parent);
		virtual ~Sprite();
		...
		//Animation Functions
		void stop(){this->paused = true;}
		void play(){this->paused = false;}
		void gotoAndStop(int frame){this->curframe = frame;this->paused = true;}
		void gotoAndPlay(int frame){this->curframe = frame;this->paused = false;}

		//screen position
		//Vector3 getPosition() { return position; }
		//void setPosition(Vector3 position) { this->position = position; }
		void setPosition(double x, double y) {body->p.x = x ;body->p.y = y ;}
		double getX() { return body->p.x ; }
		double getY() { return body->p.y ; }
		...
		//return Shape ID
		Shape*  addCircle(float radius,float xOffset,float yOffset);
		Shape*  addLineSegment(float ax,float ay,float bx,float by,float radius);
		Shape*  addConvexPolygon(int vertxCnt,cpVect *vertxs , float xOffset , float yOffset);

		Shape* findShape(int id);
		void setShapeVisible(int id , bool value);
		void drowShapes();

		void applyImplus(float i,float j) { cpBodyApplyImpulse(this->body, cpv(0,-1000),cpv(0,0)); }
