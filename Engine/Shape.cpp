...
namespace Soshiant2D {
	...
	void Shape::drow()
	{
		if( this->visible)
		{
			...
			if( this->type == ShapeTypes::LINESEGMENT)
			{
				...
				D3DXVECTOR2 lines[] = {D3DXVECTOR2( (float)A.x, (float)A.y ), D3DXVECTOR2(B.x,B.y)};
				...
				if(seg->r > 0)
				{
					float rot = atan((B.y-A.y)/(B.x-A.x)) ;
			...
			else if( this->type == ShapeTypes::CIRCLE )
			{
				...
				int i=0;
				for(float angle=0;angle<=2*3.14;angle+=2*3.14/14)
				{
					lines[i++] = D3DXVECTOR2((double)(cos(angle)*R+P.x),(double)(sin(angle)*R+P.y));
	...
	{
		D3DXVECTOR2 res(x-baceX,y-baceY);
